/*=========================================================================
  ...
  Module:    myConnectivityFilter.cxx
...
vtkObjectFactoryNewMacro(myConnectivityFilter);

// Construct with default extraction mode to extract largest regions.
myConnectivityFilter::myConnectivityFilter()
{
...
int myConnectivityFilter::RequestDataObject(vtkInformation* vtkNotUsed(request),
...
int myConnectivityFilter::RequestData(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  this->RegionSizes->InsertValue(0, 0);
  this->RegionSizes->InsertValue(1, 0);
  ...
  { // visit all cells marking with region number
    for (cellId = 0; cellId < numCells; cellId++)
    {
      ...
      if (this->Visited[cellId] < 0)
      {
		...
		if (this->NumCellsInRegion < this->minCellsInRegion)
		{
			for (vtkIdType cId = 0; cId < this->NewCellScalars->GetNumberOfTuples(); cId++)
			{
				vtkIdType rId = static_cast<vtkIdType>(this->NewCellScalars->GetValue(cId));
				if (rId == this->RegionNumber)
				{
					this->NewCellScalars->SetValue(cId, 0);
				}
			}

			for (vtkIdType pId = 0; pId < this->NewScalars->GetNumberOfTuples(); pId++)
			{
				vtkIdType rId = static_cast<vtkIdType>(this->NewScalars->GetValue(pId));
				if (rId == this->RegionNumber)
				{
					this->NewScalars->SetValue(pId, 0);
				}
			}

			int prev_region_size = static_cast<int>(this->RegionSizes->GetValue(0));
			int new_region_size = prev_region_size + this->NumCellsInRegion;
			this->RegionSizes->SetValue(0, new_region_size);
		}
		else
		{
			bool flag = false;
			vtkIdType rId = static_cast<vtkIdType>(this->NewCellScalars->GetValue(0));
			if (rId == 0)
			{
				flag = true;
			}
			...
			for (vtkIdType cId = 0; cId < this->NewCellScalars->GetNumberOfTuples(); cId++)
			{
				...
				if (rId == this->RegionNumber)
				{
					this->NewCellScalars->SetValue(cId, 1);
				}
			}
			...
			for (vtkIdType pId = 0; pId < this->NewScalars->GetNumberOfTuples(); pId++)
			{
				...
				if (rId == this->RegionNumber)
				{
					this->NewScalars->SetValue(pId, 1);
				}
			}
			...
			int prev_region_size = static_cast<int>(this->RegionSizes->GetValue(1));
			int new_region_size = prev_region_size + this->NumCellsInRegion;
			this->RegionSizes->SetValue(1, new_region_size);
		}
...
//
void myConnectivityFilter::TraverseAndMark(vtkDataSet* input)
{
...
void myConnectivityFilter::OrderRegionIds(
...
int myConnectivityFilter::GetNumberOfExtractedRegions()
{
  return this->RegionSizes->GetMaxId() + 1;
}

// Set the minimum regions size.
void myConnectivityFilter::SetMinimumRegionSize(int size)
{
	this->minCellsInRegion = size;
}


vtkTypeBool myConnectivityFilter::ProcessRequest(
...
void myConnectivityFilter::InitializeSeedList()
{
  this->Modified();
  this->Seeds->Reset();
}
...
void myConnectivityFilter::AddSeed(vtkIdType id)
{
  this->Modified();
...
void myConnectivityFilter::DeleteSeed(vtkIdType id)
{
  this->Modified();
...
void myConnectivityFilter::InitializeSpecifiedRegionList()
{
  this->Modified();
...
void myConnectivityFilter::AddSpecifiedRegion(int id)
{
  this->Modified();
...
void myConnectivityFilter::DeleteSpecifiedRegion(int id)
{
  this->Modified();
  this->SpecifiedRegionIds->DeleteId(id);
}

int myConnectivityFilter::FillInputPortInformation(int, vtkInformation* info)
{
  info->Set(vtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "vtkDataSet");
  return 1;
}

int myConnectivityFilter::FillOutputPortInformation(int vtkNotUsed(port), vtkInformation* info)
{
  info->Set(vtkDataObject::DATA_TYPE_NAME(), "vtkDataSet");
  return 1;
}

void myConnectivityFilter::PrintSelf(ostream& os, vtkIndent indent)
{
