/*=========================================================================
  ...
  Module:    myPolyDataConnectivityFilter.cxx
...
vtkStandardNewMacro(myPolyDataConnectivityFilter);

// Construct with default extraction mode to extract largest regions.
myPolyDataConnectivityFilter::myPolyDataConnectivityFilter()
{
...
int myPolyDataConnectivityFilter::RequestData(vtkInformation* vtkNotUsed(request),
    ...
    for (cellId = 0; cellId < numCells; cellId++)
    {
      ...
      if (this->Visited[cellId] < 0)
      {
		...
		if (this->NumCellsInRegion < this->minCellsInRegion)
		{
			...
			{
				vtkIdType rId = this->Visited[cId];
			...
			{
				vtkIdType rId = static_cast<vtkIdType>(this->NewScalars->GetTuple1(pId));
				if (rId == this->RegionNumber)
				{
					this->NewScalars->SetTuple1(pId, 0);
				}
			}
		...
		{
			bool flag = false;
			vtkIdType rId = this->Visited[cellId];
			if (rId == 0)
			{
				flag = true;
			}
			...
			for (vtkIdType pId = 0; pId < this->NewScalars->GetNumberOfTuples(); pId++)
			{
				...
				if (rId == this->RegionNumber)
				{
					this->NewScalars->SetTuple1(pId, 1);
				}
			}
			if (flag)
			{
...
//
void myPolyDataConnectivityFilter::TraverseAndMark()
{
...
//------------------------------------------------------------------------------
int myPolyDataConnectivityFilter::IsScalarConnected(vtkIdType cellId)
{
  double s;
...
int myPolyDataConnectivityFilter::GetNumberOfExtractedRegions()
{
  return this->RegionSizes->GetMaxId() + 1;
}

// Set the minimum regions size.
void myPolyDataConnectivityFilter::SetMinimumRegionSize(int size)
{
...
void myPolyDataConnectivityFilter::InitializeSeedList()
{
  this->Modified();
  this->Seeds->Reset();
}
...
void myPolyDataConnectivityFilter::AddSeed(int id)
{
  this->Modified();
...
void myPolyDataConnectivityFilter::DeleteSeed(int id)
{
  this->Modified();
...
void myPolyDataConnectivityFilter::InitializeSpecifiedRegionList()
{
  this->Modified();
...
void myPolyDataConnectivityFilter::AddSpecifiedRegion(int id)
{
  this->Modified();
...
void myPolyDataConnectivityFilter::DeleteSpecifiedRegion(int id)
{
  this->Modified();
  this->SpecifiedRegionIds->DeleteId(id);
}

void myPolyDataConnectivityFilter::PrintSelf(ostream& os, vtkIndent indent)
{
